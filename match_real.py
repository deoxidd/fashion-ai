...
print("=" * 65)
print("PERSONALIZED MATCHER — REAL PRODUCT DATA")
print("=" * 65)
...
if not os.path.exists("real_catalog.csv"):
    print("\nERROR: real_catalog.csv not found. Run fetch_products.py first.")
    exit()
...
print(f"Loaded {len(catalog)} real products\n")
...
def score_item(item, user_tags):
    ...
    tag_score = score_tags(item["tags"], user_tags)
...
print("\n" + "=" * 65)
print(f"TOP REAL PRODUCT MATCHES")
...
for category in ["tops", "bottoms", "outerwear", "shoes"]:
    ...
    for i, item in enumerate(top_3, 1):
        reasons_str = " | ".join(item["reasons"]) if item["reasons"] else "decent overall match"
...
print("\n" + "=" * 65)
print(f"All recommendations are from real retailers and are shoppable.")
print("=" * 65)